function Get-ModuleConfig {
    ...
    $UserPowerShellModuleConfigPath = Join-Path -Path $(Split-Path -Path $($env:PSModulePath -split ';' | ForEach-Object { if (($_ -match $env:USERNAME) -and ($_ -notmatch '\.')) {$_} }) -Parent) -ChildPath 'Config'
